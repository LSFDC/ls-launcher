...
void ioLoginDlg::OnBnClickedLogin()
{
	...
	ioLoginDlg* pDlg = (ioLoginDlg*)AfxGetApp()->GetMainWnd();
	pDlg->MinimizeAllWindows(true);


	if( m_sLoginURL.IsEmpty() )
	{
		pMainFrame->ShowErrorDlg( "LoginURL cannot be empty!", 108 );
		return;
	}
	...
	if( !pLocal->GetLoginResultPostData(szError, sizeof( szError ), (LPCTSTR) m_sLoginURL, "lguplus", "lguplus1234") )
	{
		pMainFrame->ShowErrorDlg( szError, 108 );
		return;
	}
#else
	...
	{
		pMainFrame->ShowErrorDlg( "Username cannot be empty!", 108 );
		return;
	}
	CString sPW;
	...
	{
		pMainFrame->ShowErrorDlg( "Password cannot be empty!", 108 );
		return;
	}
#ifdef SRC_KOR
	if( !pLocal->GetLoginResultPostData(szError, sizeof( szError ), (LPCTSTR) m_sLoginURL, sID, sPW) )
	{
		pMainFrame->ShowErrorDlg( szError, 108 );
		return;
	}
	...
	char* fileName = strtok(szError, " ");
	char* command = strtok(NULL, "");
	char rootDir[MAX_PATH] = "";
	::GetCurrentDirectory(MAX_PATH, rootDir);
	char szClientName[MAX_PATH] = "";

	StringCbPrintf(szClientName, sizeof(szClientName), "%s\\%s", rootDir, fileName); //copying root app and app name from REST API response

	CSetupAndStart* SetUpAndStart = new CSetupAndStart;
	
	SetUpAndStart->InitSplashScreen();
	SetUpAndStart->ExcuteProcess((LPCTSTR)szClientName, command);
...
void ioLoginDlg::MinimizeAllWindows(BOOL bMinimize)
{
	HRESULT hr;
