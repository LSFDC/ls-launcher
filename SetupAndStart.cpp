...
#pragma comment(lib, "gdiplus.lib") // Menambahkan pustaka GDI+ ke dalam proyek
...
void CSetupAndStart::InitSplashScreen() {

	GdiplusStartupInput gdiplusStartupInput;
	ULONG_PTR gdiplusToken;
	GdiplusStartup(&gdiplusToken, &gdiplusStartupInput, NULL);

	HWND hwndSplash = CreateWindow(
		"STATIC", NULL,
		WS_POPUP | WS_VISIBLE,
	...
	Image splashImage(L"lostsaga-logo-faith.png");
	graphics.DrawImage(&splashImage, 0, 0, 400, 300);
...
void CSetupAndStart::SetAppName(const char* szAppName) {
	StringCbCopy(m_szAppName, sizeof(m_szAppName), szAppName);

}

const char* CSetupAndStart::GetAppName() const {
	return m_szAppName;
}
